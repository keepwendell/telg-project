...
- 旧素材（无 `*_zh` 字段）自动降级为纯英文展示，不报错；种子素材（seed_materials.json 的 m1/m2/m3）与前端 MOCK_MATERIALS、后端模板均已补齐双语，`init_db()` 启动时对已存在但缺 zh 的种子行做幂等补齐（`sync_seed_bilingual`），旧库升级后示例素材自动获得中文对照。
